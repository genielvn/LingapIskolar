...
class DashboardController extends Controller
{
    public function index()
    {
        ...
        if ($user->isManager()) return $this->managerDashboard();
        if ($user->isAgent()) return $this->agentDashboard();
    ...
    private function adminDashboard()
    {
        ...
        return view('routes.admin-dashboard', compact('tickets', 'agents', 'stats'));
    }
    ...
    private function managerDashboard()
    {
        ...
        return view('routes.manager-ticket-dashboard', compact('tickets', 'agents', 'stats'));
    }
    ...
    private function agentDashboard()
    {
        ...
        return view('routes.agent-ticket-dashboard', compact('tickets', 'stats'));
    }
    ...
    private function userDashboard()
    {
        ...
        return view('routes.user-ticket-dashboard', compact('tickets', 'stats'));
    }

    // Helper: map tickets to array
    private function mapTickets($allTickets, $agentName = null, $user = null)
    {
        return $allTickets->map(function ($ticket) use ($agentName, $user) {
            return [
                ...
                'requested_by' => $user?->name ?? $ticket->user?->name ?? 'Unknown',
                'requestor_title' => 'Student',
                'requestor_img_link' => '/img/user1.png',
                'assigned_to' => $agentName ?? $ticket->currentAssignment?->agent?->name ?? 'Unassigned',
    ...
    {
        return User::role('agent')->get()->map(fn($agent) => [
            'id' => (string)$agent->id,
            'name' => $agent->name,
            'email' => $agent->email,
            'title' => 'Agent',
